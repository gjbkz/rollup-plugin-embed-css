...
class EmbedCSSPlugin {
    ...
    async getCSSAST(id) {
        ...
        return this.plugins
        ? (await postcss(this.plugins).process(css, processOptions)).root
    ...
    async load(id) {
        ...
        const {classes, properties, dependencies} = await this.parse(id);
        return [
            ...[...dependencies].map(([, id]) => `import '${id}';`),
    ...
    renderChunk(source) {
        ...
        s.prepend(genereateCSSScript(this.css));
        return {
            code: s.toString(),
            map: s.generateMap(),
        };
