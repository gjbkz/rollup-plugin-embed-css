...
const {getHash} = require('@nlib/node-util');
...
const getDependencies = (ast, id) => {
    ...
    ast.walkAtRules((node) => {
        ...
        .replace(/^(['"])([^\s'"]+)\1\s*([^\s]*)$/, (match, quote, target, givenName) => {
            ...
            dependencies.set(name, absolutify(target, directory));
            node.remove();
        });
    });
    return dependencies;
...
const getCSSAST = async (id, source, config) => {
    ...
    if (config.plugins) {
        return (await postcss(config.plugins).process(source, processOptions)).root;
    } else {
        return postcss.parse(source, processOptions);
    }
};
...
const parse = async (id, source, config) => {
    const css = source || await readFile(id, 'utf8');
...
module.exports = (options = {}) => {
    ...
    return {
        ...
        async buildEnd() {
            if (config.dest) {
                await updateFile(config.dest, getCSSFromCache(config.cache));
            }
        },
        renderChunk(source) {
            if (config.dest) {
